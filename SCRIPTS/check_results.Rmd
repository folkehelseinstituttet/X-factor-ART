...
```{r volcano_plots_model1, fig.show='hide', fig.keep='none'}
volcano_pl_m1_b4b <- ewas_results_tbl %>%
  filter(model == "Model 1") %>%
  ggplot(aes(effect_size, -log10(old_ps))) +
...
volcano_pl_m1_a4b <- ewas_results_tbl %>%
  filter(model == "Model 1") %>%
  ggplot(aes(effect_size, -log10(ps))) +
...
(volcano_pl_m1_b4b + volcano_pl_m1_a4b) + plot_layout(guides = "collect")
ggsave(
  here("FIGURES", "volcano_plots_m1.png"),
  width = 10,
  height = 6
)
```
