...
library(gghighlight)
library(ggiraph)
library(dmrff) # installed from github
...
if(!file.exists(genes_dmrs_file)){
  genes_dmrff <- map(dmrff_signif_list, function(cur_dmrff){
    ...
    cur_genes <- grabGenes(
      data_in, chromosome = "chrX", asGRanges = TRUE
    )
    return(cur_genes)
  })
...
> This table needs to be built and saved on its own.

The newest version is saved here as [nice_table_signif_dmrs_all.html](nice_table_signif_dmrs_all.html).
...
signif_results_gen_location_gt <- gt(
    signif_results_gen_location %>%
      dplyr::select(!c(chr, B, S, z, se, p.value)) %>%
      dplyr::mutate(
  ...
  tab_header(
    title = md("**Most significant DMRs (FDR < 0.01 and #CpGs >= 3)**"),
    subtitle = md("_(grouped by sex and model)_")
  ) %>%
  tab_spanner(
    label = md("**DMR info**"),
    columns = c(Name, n, estimate, p.adjust)
  ) %>%
  tab_spanner(
  ...
  tab_style(
    ...
    locations = cells_body(
      columns = c(p.adjust, region_position)
    )
  ) %>%
  data_color(
    columns = estimate,
  ...
  tab_footnote(
    ...
    locations = cells_column_labels(
      columns = c("gene_position", "Name", "region_position")
    )
  )
# signif_results_gen_location_gt
```

```{r nice_table_flextable, eval = FALSE}
...
save_as_html(
  signif_results_gen_location_flex,
  path = here("RESULTS", "flextable_signif_dmrs.html"),
...
walk(names(regul_regs_dmrff), function(name){
  ...
  write_delim(
    as_tibble(regul_regs_dmrff[[name]]) %>%
      dplyr::select(reg_ID, chr = seqnames, start, end, type) %>%
      distinct(),
...
regul_regs_dmrff_all_grouped <- regul_regs_dmrff_all %>%
  ...
  left_join(
    regul_regs_dmrff_all %>%
      dplyr::select(reg_ID, chr, start, end, type) %>%
      distinct()
  )
